...
dpkg -l $GANGLIA_PACKAGES | grep "Status" > temp.txt

if ! [[ -s temp.txt]]; then
  sudo apt-get install -q -y $GANGLIA_PACKAGES;
fi
for node in $SLAVES $OTHER_MASTERS; do
  ssh -t -t $SSH_OPTS root@$node "dpkg -l $GANGLIA_PACKAGES | grep "Status" > temp.txt; if ! [[ -s temp.txt]]; then sudo apt-get install -q -y $GANGLIA_PACKAGES; fi" & sleep 0.3
done
wait
